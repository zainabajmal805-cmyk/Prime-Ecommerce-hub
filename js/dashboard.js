...
if (addStudentForm) {
  addStudentForm.addEventListener('submit', function(e) {
    ...
    if (tbody) {
      ...
      // Update overview student count
      updateStatCount('students', +1);
    }
...
function updateAdmStatus(name, status, cls, btn) {
  ...
  if (status === 'Approved') updateStatCount('admissions', +1);
}

// Add approved student to Students section
function addApprovedToStudents(app) {
  const tbody = document.querySelector('#std-table tbody');
  if (!tbody) return;
  if (tbody.querySelector(`[data-txn="${app.txn}"]`)) return; // no duplicate

  const rows   = tbody.querySelectorAll('tr').length;
  const sid    = '#S' + String(rows + 1).padStart(3, '0');
  const tr     = document.createElement('tr');
  ...
  tr.innerHTML = `
    ...
    <td>${app.date}</td>
    <td><span class="db-pill green">Active</span></td>
    <td><button class="db-btn-sm" onclick="openStudentProfile(this,'${app.name}','${sid}','${app.course}','${app.phone}','${app.email}','${app.date}')">Profile</button></td>`;
  tbody.appendChild(tr);

  // Update total students count
  updateStatCount('students', +1);
  showToast(`${app.name} added to Students list`);
}
    ...
    if (actTbody && i < 5) {
      if (!actTbody.querySelector(`[data-txn="${app.txn}"]`)) {
        ...
        tr.innerHTML = `
          ...
          <td>${app.course}</td>
          <td>${app.date}</td>
          <td><span class="db-pill amber">Pending</span></td>`;
